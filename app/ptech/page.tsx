...
export default async function PTechPage() {
  ...
  // 1. The GROQ Query: Ask Sanity for all milestones
  ...
  // 2. Fetch the data
  const rawMilestones = await client.fetch(query);

  // 3. Sort them correctly (since 9 comes after 10 in standard alphabetical string sorting, we force the math here)
  const sortMap: Record<string, number> = { '9': 1, '10': 2, '11': 3, '12': 4, '13': 5, '14': 6 };
  const milestones = rawMilestones.sort((a: any, b: any) => sortMap[a.gradeLevel] - sortMap[b.gradeLevel]);
  ...
  return (
    <main className="flex min-h-screen flex-col items-center pb-20 bg-brand-light">
      ...
      <section className="w-full py-16 px-6 max-w-4xl mx-auto border-b border-brand-dark/10 mb-12">
        <Link href="/" className="text-brand-slate font-mono text-sm hover:text-brand-dark transition-colors mb-8 inline-block">
          ← BACK TO HOME
        </Link>
        ...
        <p className="text-lg text-brand-slate">
          Track the exact academic and technical milestones our students conquer from freshman year to their Associate's Degree.
        </p>
      </section>
      ...
      <section className="w-full px-6 max-w-3xl mx-auto relative">
        ...
        {/* The Vertical Line */}
        <div className="absolute left-[39px] md:left-[50px] top-0 bottom-0 w-0.5 bg-brand-dark/10"></div>

        <div className="flex flex-col gap-12">
          {milestones.length > 0 ? (
            milestones.map((milestone: any) => (
              <div key={milestone._id} className="relative flex items-start gap-6 md:gap-10 group">
...
{/* --- ECI CALL TO ACTION --- */}
      <section className="w-full py-24 px-6 bg-[#004B87] text-white border-t-8 border-brand-primary relative overflow-hidden">
        {/* Subtle background pattern to keep it from being a flat wall of color */}
        ...
        <div className="max-w-4xl mx-auto text-center flex flex-col items-center relative z-10">
          ...
          <p className="text-xl md:text-2xl text-blue-100 mb-12 font-light max-w-2xl">
            Dual enroll with <strong className="text-white font-bold">City Tech</strong> and start earning your Associate's Degree for <span className="text-brand-primary font-bold underline decoration-4 underline-offset-4">free</span>.
          </p>
          ...
          >
            EXPLORE ECI PATHWAYS <span>→</span>
          </Link>
        </div>
      </section>
    </main>
