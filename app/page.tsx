...
export default async function HomePage() {
  ...
  const announcements = await client.fetch<Announcement[]>(query);
          ...
          <div className="flex flex-col items-start">
            <h1 className="text-5xl md:text-7xl font-bold text-brand-dark tracking-tight mb-6 leading-tight">
              ...
              <span className="relative inline-block">
                <span className="absolute bottom-2 left-0 w-full h-1/2 bg-brand-primary -z-10" />
                Future.
              </span>
            </h1>
            <p className="text-xl text-brand-slate max-w-lg mb-10">
              City Polytechnic High School of Engineering, Architecture, and
              Technology. Building the next generation of innovators in downtown
              Brooklyn.
            </p>
            <div className="flex flex-wrap gap-4">
              <Link
                href="/about/about-city-poly"
                className="bg-brand-primary text-brand-dark font-bold font-mono text-sm px-6 py-3 border-2 border-brand-primary hover:shadow-lg transition-all inline-block"
              >
                EXPLORE P-TECH
              </Link>
              <Link
                href="/portfolio"
                className="bg-transparent text-brand-dark font-bold font-mono text-sm px-6 py-3 border-2 border-brand-dark hover:bg-brand-dark hover:text-brand-light transition-all inline-block"
              >
                STUDENT PORTFOLIOS
              </Link>
            </div>
            ...
            announcements.map((post) => (
              ...
              >
                ...
                <div className="mt-6 pt-4 border-t border-brand-dark/10 flex justify-between items-center text-xs font-mono text-brand-slate">
                  ...
                  <span className="group-hover:translate-x-2 transition-transform font-bold text-brand-primary">
                    -&gt;
                  </span>
                </div>
              </Link>
